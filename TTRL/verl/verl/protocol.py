...
class DataProto:
    ...
    def make_iterator(self, mini_batch_size, epochs, seed=None, dataloader_kwargs=None):
        ...
        """
        drop = self.batch.batch_size[0] % mini_batch_size ### Deal with data sizes, which are not multiples of the data
        if drop:
            self = self.slice(0, self.batch.batch_size[0] - drop)
    ...
    def chunk(self, chunks: int) -> List["DataProto"]:
        ...
        """
        drop = len(self) % chunks
        if drop:
            self = self.slice(0, len(self) - drop)
